...
public final class SakuraConfigurations extends Configurations<GlobalConfiguration, WorldConfiguration> {
    ...
    private static SakuraConfigurations instance;

    public static SakuraConfigurations getInstance() {
        return instance;
    }

    private static void setInstance(SakuraConfigurations config) {
        instance = config;
    }
    ...
    public void reloadConfigs(MinecraftServer server) {
        try {
            this.initializeGlobalConfiguration(server.registryAccess());
            this.initializeWorldDefaultsConfiguration(server.registryAccess());
            for (ServerLevel level : server.getAllLevels()) {
                this.createWorldConfig(createWorldContextMap(level));
            }
        } catch (Exception ex) {
    ...
    public static SakuraConfigurations setup(final Path configDir) {
        try {
            ...
            throw new RuntimeException("Could not setup SakuraConfigurations", ex);
        }
    }
